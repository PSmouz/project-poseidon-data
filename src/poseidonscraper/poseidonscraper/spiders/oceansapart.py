...
from poseidonscraper.items import OceansapartItem
...
class OceansapartSpider(scrapy.Spider):
    ...
    #
    # Control the logging level for Selenium and urllib3 to make console more readable. 
    #
    ...
    def __init__(self, categories=None, *args, **kwargs):
        ...
        self.start_urls = [f"https://www.oceansapart.com/en/product-category/{category_paths[cat]}" for cat in categories]
    ...
    def start_requests(self):
        ...
        for url in self.start_urls:
            yield Request(url, callback=self.parse_products , dont_filter=True, meta={'original_url': url})
    ...
    def parse_products(self, response):
        ...
        for item_url in products:
            yield response.follow(item_url, callback=self.parse_item, meta={'original_url': response.meta['original_url']})
        
        # Extract the next page link
        next_page = response.css('.next::attr(href)').extract_first()
        if next_page:
            yield response.follow(next_page, callback=self.parse_products, meta={'original_url': response.meta['original_url']})
        

    def parse_item(self, response):
        color_code = response.xpath("string(//*[contains(@class, 'oa-bubble--active')]/span//@class)").get().split(" ")[1]
        ...
        l.add_xpath("name", '/html/body/main/main/section/article/div[1]/h1/text()')
        l.add_value("category_name", response.meta['original_url'])
        l.add_xpath("images", '//*[@id="productDetailsCarousel"]//*[not(contains(@class, "tns-slide-cloned"))]/@href')
        l.add_xpath("materials", '/html/body/main/main/section/article/div[4]/div[2]/div/ul')
        l.add_xpath("sizes", '/html/body/main/main/section/article/form/div[1]/select/option/text()')
        l.add_xpath("color_name", '/html/body/main/main/section/article/div[1]/h1/span/text()')
        l.add_value("color_value", self.color_map[color_code])
        l.add_xpath("price", '/html/body/main/main/section/article/div[2]/p/span/bdi/text()')
        l.add_xpath("price", '/html/body/main/main/section/article/div[2]/p/del/span/bdi/text()') # Sale Price
        # l.add_xpath("name", '/html/body/main/main/section/article/div[1]/h1/text()')
        # l.add_value("category_name", response.meta['original_url'])
        ...
        # l.add_xpath("color_name", '/html/body/main/main/section/article/div[1]/h1/span/text()')
        # l.add_value("color_value", self.color_map[color_code])
        # l.add_value("price", product['prices']['price'])
        # l.add_value("price", product['prices']['sale_price'])
        yield l.load_item()

    def get_color_map(self):
        url = "https://cdn.oceansapart.com/de/wp-content/plugins/oceansapart-extensions/includes/color-bubbles/css/colors.css?ver=3.112.0"
...
def parse_css_colors(css):
    ...
    pattern = r'\.([\w-]+)(?:,\s*([\w-]+))?\s*{\s*background-color\s*:\s*#([0-9a-fA-F]+)(?:\s*;\s*)?}'
